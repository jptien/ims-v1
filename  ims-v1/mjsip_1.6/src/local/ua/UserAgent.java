...
public class UserAgent extends CallListenerAdapter {
	...
	public UserAgent(SipProvider sip_provider, UserAgentProfile user_profile,
		...
		// #################
		// currently AudioSender must be started before any AudioClipPlayer is
		// initialized,
		// since there is a problem with the definition of the audio format
		if (!user_profile.use_rat && !user_profile.use_jmf) {
			if (user_profile.audio && !user_profile.recv_only
					&& user_profile.send_file == null
					&& !user_profile.send_tone)
				local.media.AudioInput.initAudioLine();
			if (user_profile.audio && !user_profile.send_only
					&& user_profile.recv_file == null)
	...
	/*
	 * private void newSession(int media_port) { SessionDescriptor local_sdp=new
	 * SessionDescriptor(user_profile.from_url,sip_provider.getAddress()); int
	 * audio_port=media_port; int video_port=media_port+2; //PATCH [040902] if
	 * (audio || !video) local_sdp.addMedia(new
	 * MediaField("audio",audio_port,0,"RTP/AVP","0"),new
	 * AttributeField("rtpmap","0 PCMU/8000")); //PATCH [040902] if (video ||
	 * !(audio || video)) local_sdp.addMedia(new
	 * MediaField("video",video_port,0,"RTP/AVP","7"),new
	 * AttributeField("rtpmap","17")); local_sdp.addMedia(new
	 * MediaField("audio",audio_port,0,"RTP/AVP","0"),new
	 * AttributeField("rtpmap","0 PCMU/8000"));
	...
	protected void launchMediaApplication() {
		...
		if (user_profile.audio && local_audio_port != 0
			...
			if (audio_app == null) { // for testing..
				...
				// audio_app=new
				// JAudioLauncher(local_audio_port,remote_media_address,remote_audio_port,dir,log);
	...
	public void onCallAccepted(Call call, String sdp, Message resp) {
		...
		if (user_profile.no_offer) { // Create the new SDP
			SessionDescriptor remote_sdp = new SessionDescriptor(sdp);
			SessionDescriptor local_sdp = new SessionDescriptor(local_session);
			SessionDescriptor new_sdp = new SessionDescriptor(remote_sdp
					.getOrigin(), remote_sdp.getSessionName(), local_sdp
